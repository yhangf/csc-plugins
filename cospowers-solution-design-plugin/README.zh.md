...
- 在多轮设计过程中保存或恢复会话上下文，便于后续任务拆解和实现交接。
...
- `docs/design/design-change-impact.md`：设计变更影响分析报告。
- `docs/design/doc-consistency-report.md`：文档一致性检查报告。
- 会话上下文摘要或归档说明：记录设计背景、决策、假设、风险和后续交接信息。
...
5. 如果要继续推进，可以把设计文档和会话上下文交给任务拆解、测试生成或开发流程使用。
...
| `design-master-perspective` | 从设计质量视角诊断风险、边界、拆分、抽象和反模式。 | 系统或子系统设计决策需要更深入审视时使用。 |
...
| `session-context` | 整理、恢复或归档当前设计会话上下文。 | 会话较长，需要保留背景、决策和未解决问题。 |
