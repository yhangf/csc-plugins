...
- 会话上下文摘要或归档说明：记录背景、决策、假设、开放问题和后续交接信息。
...
2. 调用 `requirements-intake`，说明你希望整理新需求、评审需求、分析需求变更，还是整理会话上下文。
3. 根据插件追问补充背景，例如目标用户、业务规则、边界条件、成功标准。
4. 查看输出的需求文档或评审报告，确认是否符合真实业务意图。
5. 如果要继续推进，可以把需求文档和会话上下文交给方案设计、任务拆解、测试生成或开发流程使用。
...
| `session-context` | 整理、恢复或归档当前需求梳理会话上下文。 | 会话较长，需要保留背景、决策和未解决问题。 |
